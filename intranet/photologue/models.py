...
class ImageModel(models.Model):
    ...
    def cache_url(self):
        #import re
        #m = re.match(settings.MEDIA_URL+'(.*)', os.path.dirname(self.image.url))
        #return '/'.join([settings.MEDIA_URL, 'upload', m.group(1), "cache"])
...
class Photo(ImageModel):
    ...
    title_slug = models.SlugField(_('slug'), blank=True, null=True,
